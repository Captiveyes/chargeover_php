...
require_once 'ChargeOverException.php';
require_once 'ChargeOverExceptionFeatureNameExists.php';
...
class ChargeOverAPI
{
    ...
    protected function _request($http_method, $uri, $data = null)
    {
        ...
        {
            // Signed requests
            $headers[] = $this->_signature($this->_username, $this->_password, $endpoint, $post_data);
        }
        ...
        {
            curl_close($ch);
            throw new ChargeOverException(
                'Problem hitting URL [' . $endpoint . ']: ' . curl_error($ch) . ', ' . print_r($this->_last_info, true)
            );
        }
        ...
        if (json_last_error() != JSON_ERROR_NONE) {
            throw new ChargeOverException(
                ...
                . $out . ', JSON parser returned error: ' . json_last_error()
            );
        }
        // We at least got back a valid JSON object
        if ($data->status != ChargeOverAPI::STATUS_OK)
        {
            if (substr_count($data->message, \ChargeOverExceptionFeatureNameExists::FEATURE_NAME_EXISTS)) {
               throw new ChargeOverExceptionFeatureNameExists($data->message);
            } else {
                throw new ChargeOverException($data->code . ': ' . $data->message);
            }
        }
     ...
     * @param string                      $method
     * @param integer                     $id
     * @param ChargeOverApi_Object|string $Object_or_obj_type            Either an object, or an object type constant
     * @return string                                                     The URL for the API
     */
    protected function _map($method, $id, $Object_or_obj_type)
    {
        switch($method) {
            case ChargeOverAPI::METHOD_AGGREGATE:
                return '_aggregate';
            case ChargeOverAPI::METHOD_BULK:
                return '_bulk';
            case ChargeOverAPI::METHOD_CONFIG:
                return '_config';
        }
    ...
    /**
     * @param ChargeOverAPI_Object|string $Object_or_class
     *
     * @return null
     */
    ...
    /**
     * @param ChargeOverAPI_Aggregate $Aggregate
     *
     * @return mixed|null
     */
    public function aggregate(ChargeOverAPI_Aggregate $Aggregate)
    {
     ...
     * @param integer $id
     * @param string $action
     * @param ChargeOverApi_Object|array $data
     * @return array
     */
    public function action($type, $id, $action, $data = array())
    {
        $uri = $this->_map(ChargeOverAPI::METHOD_ACTION, $id, $type) . '?action=' . $action;
    ...
    /**
     * @param ChargeOverAPI_Object $Object
     *
     * @return mixed|null
     */
    public function create(ChargeOverAPI_Object $Object)
    {
     ...
     * @return mixed|null
     */
    public function modify($id, ChargeOverAPI_Object $Object)
    {
     ...
     *
     * @return mixed
     * @throws ChargeOverException
     */
    public function find($type, $where = array(), $sort = array(), $offset = 0, $limit = null)
    {
        $uri = $this->_map(ChargeOverAPI::METHOD_FIND, null, $type) . '?_dummy=1';
    ...
    public function findById($type, $id)
    {
        if (!$id)
        {
            throw new ChargeOverException('You must provide a valid id value to findById($type, $id)');
        }
